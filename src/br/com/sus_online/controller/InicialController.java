...
public class InicialController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// Se n�o encontrou usuario no banco, redireciona para a pagina de erro!
		if (user == null) {
			session.invalidate();
...
//Metodo de valida��o de senha e usuario caso fuja da regra 
	private void ValidarUsuarioSenha(HttpServletRequest request, HttpServletResponse response, HttpSession session,
		...
		if (login_form == null || login_form.isEmpty() || login_form.length() > 15 || login_form.length() < 6) {
			session.invalidate();
			request.setAttribute("mensagem", "Usu�rio inv�lido; usu�rio deve ter entre 6 e 15 caracteres.");
		...
		for (int i = 0; i < login_form.length(); i++) {
			...
			if (!(Character.isLetter(caracter) || Character.isDigit(caracter))) {
				...
				request.setAttribute("mensagem",
						"Usu�rio inv�lido: usu�rio deve conter apenas letras, n�meros ou caracteres '.' e '_'.");
				request.getRequestDispatcher("view/mensagem.jsp").forward(request, response);
			}
		}
		...
		if (senha_form == null || senha_form.isEmpty() || senha_form.length() > 10 || senha_form.length() < 6) {
			session.invalidate();
			request.setAttribute("mensagem", "Senha inv�lida: senha deve ter entre 6 e 15 caracteres.");
			request.getRequestDispatcher("view/mensagem.jsp").forward(request, response);
		}
		int contLetra = 0;
		int contNumero = 0;
		for (int i = 0; i < senha_form.length(); i++) {
			...
			if (!(Character.isLetter(caracter) || Character.isDigit(caracter) || caracter == '_' || caracter == '.')) {
				session.invalidate();
				request.setAttribute("mensagem", "Senha inv�lida; senha deve conter apenas letras e n�meros.");
				request.getRequestDispatcher("view/mensagem.jsp").forward(request, response);
			} else {
		...
		if (contLetra == 0 || contNumero == 0) {
			session.invalidate();
			request.setAttribute("mensagem", "Senha inv�lida; senha deve conter letras e n�meros.");
			request.getRequestDispatcher("view/mensagem.jsp").forward(request, response);
		}
	}
